...
func (u *claudeResponsesUsageTokens) Merge(usage gjson.Result) {
	...
	if reasoningTokens := usage.Get("output_tokens_details.thinking_tokens"); reasoningTokens.Type == gjson.Number {
...
func (u claudeResponsesUsageTokens) OpenAIReasoningTokens(reasoningTextLength int) (int64, bool) {
	if u.HasReasoningTokens {
		return u.ReasoningTokens, true
	}
	estimate := int64(reasoningTextLength / 4)
...
func setResponsesRawEchoValue(out []byte, path string, value gjson.Result) []byte {
	if !value.Exists() || value.Raw == "" {
		return out
	}
	raw := []byte(value.Raw)
	// Responses SSE payloads must stay on one data line. Compacting removes
	// request formatting whitespace while preserving exact JSON number lexemes.
	var compact bytes.Buffer
	if err := json.Compact(&compact, raw); err == nil {
		raw = compact.Bytes()
	}
	updated, err := sjson.SetRawBytes(out, path, raw)
	if err != nil {
		return out
	}
	return updated
	...
	case "message_stop":
		...
		if len(reqBytes) > 0 {
			...
			if v := req.Get("reasoning"); v.Exists() {
				completed = setResponsesRawEchoValue(completed, "response.reasoning", v)
			}
			...
			if v := req.Get("text"); v.Exists() {
				completed = setResponsesRawEchoValue(completed, "response.text", v)
			}
			if v := req.Get("tool_choice"); v.Exists() {
				completed = setResponsesRawEchoValue(completed, "response.tool_choice", v)
			}
			if v := req.Get("tools"); v.Exists() {
				completed = setResponsesRawEchoValue(completed, "response.tools", v)
			}
			...
			if v := req.Get("user"); v.Exists() {
				completed = setResponsesRawEchoValue(completed, "response.user", v)
			}
			if v := req.Get("metadata"); v.Exists() {
				completed = setResponsesRawEchoValue(completed, "response.metadata", v)
			}
		}
		...
		reasoningTokens, hasReasoningTokens := st.Usage.OpenAIReasoningTokens(reasoningLength)
		usagePresent := st.Usage.HasUsage || hasReasoningTokens
		if usagePresent {
...
func ConvertClaudeResponseToOpenAIResponsesNonStream(_ context.Context, _ string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, _ *any) []byte {
	...
	// Keep output slots in the order in which Claude starts content blocks.
	// Adjacent text blocks share one message until a tool/reasoning boundary,
	// matching the streaming translator's item lifecycle.
	var (
		outputs            []outputState
	...
	startMessage := func() *messageState {
		if currentMessage != nil {
			return currentMessage
		}
		message := &messageState{
			id:          fmt.Sprintf("msg_%s_%d", responseID, messageCount),
			annotations: append([]any(nil), pendingAnnotations...),
		}
		messageCount++
		pendingAnnotations = nil
		outputs = append(outputs, outputState{message: message})
		currentMessage = message
		return message
	}
	startTool := func(index int) *toolState {
		if tool := toolCalls[index]; tool != nil {
			return tool
		}
		tool := &toolState{}
		toolCalls[index] = tool
		outputs = append(outputs, outputState{tool: tool})
		return tool
	}
	...
	for _, ch := range chunks {
		...
		case "content_block_start":
			...
			case "tool_use":
				...
				tool := startTool(idx)
				tool.id = cb.Get("id").String()
				tool.name = cb.Get("name").String()
			case "thinking":
				currentMessage = nil
				currentReasoning = &reasoningState{id: fmt.Sprintf("rs_%s_%d", responseID, idx)}
				if signature := cb.Get("signature"); signature.Exists() && signature.String() != "" {
					currentReasoning.signature = signature.String()
				}
				outputs = append(outputs, outputState{reasoning: currentReasoning})
			}
		...
		case "content_block_delta":
			...
			case "text_delta":
				if t := d.Get("text"); t.Exists() {
					if currentMessage != nil {
						currentMessage.text.WriteString(t.String())
					}
				}
			case "input_json_delta":
				if pj := d.Get("partial_json"); pj.Exists() {
					idx := int(root.Get("index").Int())
					startTool(idx).args.WriteString(pj.String())
				}
			case "thinking_delta":
				if currentReasoning != nil {
					if t := d.Get("thinking"); t.Exists() {
						currentReasoning.text.WriteString(t.String())
					}
				}
			...
			case "citations_delta":
				if citation := d.Get("citation"); citation.Exists() {
					if currentMessage != nil {
						currentMessage.annotations = append(currentMessage.annotations, citation.Value())
					} else {
	...
	if len(reqBytes) > 0 {
		...
		if v := req.Get("reasoning"); v.Exists() {
			out = setResponsesRawEchoValue(out, "reasoning", v)
		}
		if v := req.Get("safety_identifier"); v.Exists() {
		...
		if v := req.Get("text"); v.Exists() {
			out = setResponsesRawEchoValue(out, "text", v)
		}
		if v := req.Get("tool_choice"); v.Exists() {
			out = setResponsesRawEchoValue(out, "tool_choice", v)
		}
		if v := req.Get("tools"); v.Exists() {
			out = setResponsesRawEchoValue(out, "tools", v)
		}
		if v := req.Get("top_logprobs"); v.Exists() {
		...
		if v := req.Get("user"); v.Exists() {
			out = setResponsesRawEchoValue(out, "user", v)
		}
		if v := req.Get("metadata"); v.Exists() {
			out = setResponsesRawEchoValue(out, "metadata", v)
		}
	}
	...
	for _, state := range outputs {
		switch {
		case state.reasoning != nil:
			reasoningLength += state.reasoning.text.Len()
			item := []byte(`{"id":"","type":"reasoning","encrypted_content":"","summary":[]}`)
			item, _ = sjson.SetBytes(item, "id", state.reasoning.id)
			item, _ = sjson.SetBytes(item, "encrypted_content", state.reasoning.signature)
			summary := []byte(`{"type":"summary_text","text":""}`)
			summary, _ = sjson.SetBytes(summary, "text", state.reasoning.text.String())
			item, _ = sjson.SetRawBytes(item, "summary.-1", summary)
			outputsWrapper, _ = sjson.SetRawBytes(outputsWrapper, "arr.-1", item)

		case state.message != nil:
			item := []byte(`{"id":"","type":"message","status":"completed","content":[{"type":"output_text","annotations":[],"logprobs":[],"text":""}],"role":"assistant"}`)
			item, _ = sjson.SetBytes(item, "id", state.message.id)
			item, _ = sjson.SetBytes(item, "content.0.text", state.message.text.String())
			if len(state.message.annotations) > 0 {
				item, _ = sjson.SetBytes(item, "content.0.annotations", state.message.annotations)
			}
			outputsWrapper, _ = sjson.SetRawBytes(outputsWrapper, "arr.-1", item)

		case state.tool != nil:
			args := state.tool.args.String()
			if args == "" {
				args = "{}"
			}
			item := []byte(`{"id":"","type":"function_call","status":"completed","arguments":"","call_id":"","name":""}`)
			item, _ = sjson.SetBytes(item, "id", fmt.Sprintf("fc_%s", state.tool.id))
			item, _ = sjson.SetBytes(item, "arguments", args)
			item, _ = sjson.SetBytes(item, "call_id", state.tool.id)
			item = applyResponsesFunctionCallNamespaceFields(item, reqBytes, state.tool.name, "")
			outputsWrapper, _ = sjson.SetRawBytes(outputsWrapper, "arr.-1", item)
		}
	}
	if gjson.GetBytes(outputsWrapper, "arr.#").Int() > 0 {
	...
	if reasoningTokens, hasReasoningTokens := usageTokens.OpenAIReasoningTokens(reasoningLength); hasReasoningTokens {
